-- Canonical value-authority schema for production GerChain.
-- This migration intentionally fails if legacy escrow rows exist because their
-- missing currency/refund destination cannot be inferred safely.
...
BEGIN
    IF EXISTS (SELECT 1 FROM escrows) THEN
        RAISE EXCEPTION
            'canonical escrow migration requires explicit reconciliation of existing legacy escrows';
    END IF;
END $$;
