-- Canonical GerChain value authority schema.
-- Extends the legacy escrow table in-place and creates the single production
-- Ledger, witness, idempotency and transactional outbox stores.
...
ALTER TABLE escrows
    DROP CONSTRAINT IF EXISTS escrows_amount_check;

ALTER TABLE escrows
    ADD CONSTRAINT escrows_amount_check CHECK (amount >= 0);
