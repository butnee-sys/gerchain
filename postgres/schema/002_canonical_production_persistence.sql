-- Canonical production persistence schema upgrade.
-- This migration upgrades the legacy escrow table and creates the canonical
-- Ledger + Witness + Outbox + Idempotency tables used by production runtime.
...
ALTER TABLE escrows ADD COLUMN IF NOT EXISTS updated_at_v2 TIMESTAMPTZ;

UPDATE escrows
SET updated_at_v2 = COALESCE(updated_at, now())
WHERE updated_at_v2 IS NULL;

ALTER TABLE escrows ALTER COLUMN updated_at_v2 SET NOT NULL;
...
ALTER TABLE escrows
    RENAME COLUMN updated_at TO legacy_updated_at;

ALTER TABLE escrows
    RENAME COLUMN updated_at_v2 TO updated_at;
...
CREATE TABLE IF NOT EXISTS gerchain_ledger_accounts (
    ...
    CONSTRAINT ck_ledger_account_balance_nonnegative CHECK (balance >= 0)
);
...
CREATE TABLE IF NOT EXISTS gerchain_ledger_movements (
    ...
    CONSTRAINT ck_ledger_movement_amount_positive CHECK (amount > 0)
);
