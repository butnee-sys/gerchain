-- Canonical production persistence for EAI value-flow execution.
-- Migration is additive and preserves the single authoritative Ledger,
-- Escrow, Witness, Outbox and Idempotency boundaries.
...
BEGIN
    IF EXISTS (SELECT 1 FROM escrows WHERE currency IS NULL) THEN
        RAISE EXCEPTION 'canonical escrow migration requires currency for every existing escrow';
    END IF;
END $$;
...
BEGIN
    ...
    LOOP
        EXECUTE format('ALTER TABLE escrows DROP CONSTRAINT %I', c);
    END LOOP;
END $$;
