-- Canonical production persistence contract.
-- Adds the durable authorities used by GerchainRuntime canonical value flow.
-- Safe for databases already initialized by 001_concurrency.sql.
...
CREATE TABLE IF NOT EXISTS gerchain_ledger_movements (
    ...
    CONSTRAINT gerchain_ledger_movements_amount_positive CHECK (amount > 0)
);
...
-- Existing legacy rows are retained; canonical runtime writes only the
-- canonical tables above. No value is moved by this migration.
