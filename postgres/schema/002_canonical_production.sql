-- Canonical GerChain production persistence upgrade.
-- Extends the legacy escrow table to the frozen lifecycle and creates the
-- single authoritative value/evidence tables used by the production runtime.
...
CREATE INDEX IF NOT EXISTS ix_gerchain_outbox_lease
    ON gerchain_outbox_events (lease_until);
