...
-- Version 2 extends the legacy escrow table in-place and creates the
-- authoritative Ledger, Witness, Outbox and Idempotency stores.
-- Existing escrow rows must have an explicit currency before this migration
-- can complete; the migration never invents financial metadata.
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM escrows WHERE currency IS NULL OR currency = '') THEN
        RAISE EXCEPTION
            'canonical escrow migration requires explicit currency for every existing escrow row';
    END IF;
END $$;
