...
class SHUUDPersistence:
    ...
    def save_snapshot(self, incident_id: str, snapshot: dict[str, Any]) -> None:
        ...
        with self.SessionLocal() as session:
            ...
            if row is None:
                session.add(SHUUDStateRow(incident_id=incident_id, snapshot_json=encoded))
            else:
                row.snapshot_json = encoded
            session.commit()

    def save_snapshot_if_current(self, incident_id: str, expected_snapshot: dict[str, Any], snapshot: dict[str, Any]) -> bool:
        if not incident_id or not incident_id.strip():
            raise ValueError("incident_id is required")
        ...
        with self.SessionLocal() as session:
            result = session.execute(
                ...
                .where(SHUUDStateRow.incident_id == incident_id,
    ...
    def save_economic_measurement_if_current(self, incident_id: str, expected_snapshot: dict[str, Any], economic_measurement: dict[str, Any]) -> bool:
    ...
    def save_sandbox_config(self, sandbox_id: str, config: dict[str, Any]) -> None:
        ...
        with self.SessionLocal() as session:
            ...
            if row is None:
                session.add(SHUUDSandboxConfigRow(sandbox_id=sandbox_id, config_json=encoded))
            else:
    ...
    def load_sandbox_config(self, sandbox_id: str) -> dict[str, Any] | None:
        with self.SessionLocal() as session:
            ...
            if not isinstance(value, dict):
                raise ValueError("persisted SHUUD sandbox config must be a dictionary")
            return value
    ...
    def recover_witness(self, incident_id: str):
