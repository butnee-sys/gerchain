...
function money(value) { return `${Number(value || 0).toFixed(0)} MNT`; }
function minutes(value) { return `${Number(value || 0).toFixed(1)} мин`; }
...
function renderDurableKpi(kpi) {
  ...
  document.getElementById("insurerBenefit").textContent = money(kpi.total_insurer_savings_mnt);
  ...
  document.getElementById("scopeCoverage").textContent = pct(kpi.economic_measurement_coverage_rate);
  ...
  document.getElementById("kpiMeta").textContent = `Durable: ${kpi.measured_clearance_cases ?? 0} clearance measurement • ${kpi.economic_measurement_cases ?? 0} economic measurement • coverage ${pct(kpi.economic_measurement_coverage_rate)} • scope ${kpi.scope || "sandbox"}`;
}
