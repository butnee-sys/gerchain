...
def test_locked_escrow_to_governed_settlement_and_audit():
    ...
    gateway.register(connector, credential="terminal-e2e-secret", allowed_operations={"create_witness_chain", "create_escrow"})

    authorize_gateway_access(root=root, request=GatewayAccessRequest("I2B-TERMINAL-E2E", "EXIM", "create_witness_chain", "REQ-TERM-WIT", root.owner_id, "nonce-term-wit"), trinity_proof=trinity)
    witness = gateway.dispatch("EXIM", "create_witness_chain", credential="terminal-e2e-secret", nonce="nonce-term-wit", initial_state={"case_id": "CASE-TERM"}, manifest={"case_id": "CASE-TERM", "version": 1}, witness_id="W-TERM")

    authorize_gateway_access(root=root, request=GatewayAccessRequest("I2B-TERMINAL-E2E", "EXIM", "create_escrow", "REQ-TERM-ESC", root.owner_id, "nonce-term-esc"), trinity_proof=trinity)
    escrow = gateway.dispatch("EXIM", "create_escrow", credential="terminal-e2e-secret", nonce="nonce-term-esc", escrow_id="ESC-TERM", amount=2_000_000, currency="MNT", settlement_provider="NEF", witness_chain=witness)
    ...
    assert verifier.verify_bundle(witness_bundle)
    witness_state_root = verifier.compute_state_root(witness_bundle)
    assert witness_state_root is not None
    assert verifier.verify_no_fork([witness_bundle])
    ...
    record = money.atomic_settlement(
        transaction_id="TX-TERM-1", target_state="RELEASED", source="ESCROW_POOL", destination="BENEFICIARY", amount=2_000_000,
        timestamp="2026-09-14T00:00:12Z", evidence={"case_id": "CASE-TERM", "decision": "RELEASE"}, root=root,
        owner_id=root.owner_id, authorized=True, evidence_verified=True, witness_state_root=witness_state_root, trinity_proof=trinity,
    )
    ...
    first = append_record(sequence=1, event="GATEWAY_AUTHORIZED", change_id="REQ-TERM-WIT", owner_id=root.owner_id, decision="ALLOW", stage="GATEWAY", connector_id="EXIM", request_id="REQ-TERM-WIT", operation="create_witness_chain", trinity=trinity)
    second = append_record(sequence=2, event="ATOMIC_SETTLEMENT_RELEASED", change_id="TX-TERM-1", owner_id=root.owner_id, decision="ALLOW", stage="SETTLEMENT", connector_id="EXIM", request_id="REQ-TERM-ESC", operation="atomic_settlement", previous_hash=first.record_hash, witness_state_root=witness_state_root, trinity=trinity)
...
def test_governed_release_signature_is_real_and_manifest_bound():
    ...
    manifest = build_manifest(version=1, commit_sha="E2E-COMMIT", protected_paths=protected_paths, artifact_hashes={"dee_security/e2e_governance.py": hashlib.sha256(b"e2e").hexdigest()})
    release = sign_release(private_key, owner_id=root.owner_id, release_id="REL-TERMINAL-E2E", commit_sha=manifest["commit_sha"], manifest_hash=manifest["manifest_hash"])
