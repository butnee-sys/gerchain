...
def engine():
    ...
    with engine.begin() as conn:
        ...
        conn.execute(text("DROP TABLE IF EXISTS core_schema_upgrade CASCADE")); conn.execute(text("DROP TABLE IF EXISTS core_schema_state CASCADE"))
...
def test_reader_sees_only_committed_migration_state(engine):
    ...
    with Session(engine) as session:
        tx = session.begin(); execute_migration(session, definition)
...
def test_committed_authority_without_physical_schema_is_not_accepted(engine):
    ...
    with Session(engine) as session:
        with session.begin():
            session.add(CoreSchemaUpgradeModel(upgrade_id=definition.identity.migration_id, schema_id="CORE", from_version=1, to_version=2, migration_hash=definition.identity.migration_hash, authority=definition.authority, status="APPLIED", created_at=datetime.utcnow(), applied_at=datetime.utcnow()))
            state = session.get(CoreSchemaStateModel, "CORE"); state.current_version = 2
    with Session(engine) as session:
