...
def _definition(engine, migration_id="m-1", sql=None, expected_hash=None, from_version=1, to_version=2):
    ...
    if expected_hash is None:
        with engine.begin() as conn:
            ...
            expected_hash = physical_schema_fingerprint(observe(conn, "CORE"))
...
def _authority(engine):
    with Session(engine) as session:
        return session.get(CoreSchemaStateModel, "CORE"), session.query(CoreSchemaUpgradeModel).all()

def test_transactional_success_updates_physical_and_authority(engine):
    definition = _definition(engine)
    with Session(engine) as session:
        with session.begin(): result = execute_migration(session, definition)
    state, upgrades = _authority(engine)
    assert result.status.value == "APPLIED" and state.current_version == 2 and len(upgrades) == 1
    with engine.connect() as conn: assert conn.execute(text("SELECT to_regclass('core.migration_target')")).scalar() == 'core.migration_target'
...
def test_same_id_changed_hash_is_rejected(engine):
    ...
    conflict = MigrationDefinition(MigrationIdentity("m-1", "CORE", 1, 2, migration_hash(changed_sql)), changed_sql, definition.expected_schema_hash)
    with Session(engine) as session:
        with session.begin():
            with pytest.raises(Exception, match="migration identity conflict"): execute_migration(session, conflict)
...
def test_distinct_upgrade_from_stale_predecessor_is_rejected_before_ddl(engine):
    ...
    second = MigrationDefinition(MigrationIdentity("m-2", "CORE", 1, 2, migration_hash(second_sql)), second_sql, definition.expected_schema_hash)
    with Session(engine) as session:
        with session.begin():
            with pytest.raises(MigrationExecutionError, match="stale schema predecessor"): execute_migration(session, second)
    with engine.connect() as conn: assert conn.execute(text("SELECT to_regclass('core.migration_target_two')")).scalar() is None

def test_ddl_failure_rolls_back_physical_schema_and_authority(engine):
    statements = ("CREATE TABLE core.migration_target (id bigint PRIMARY KEY)", "CREATE TABLE core.migration_target (id bigint PRIMARY KEY)")
    ...
    with engine.connect() as conn: assert conn.execute(text("SELECT to_regclass('core.migration_target')")).scalar() is None
...
def test_bypass_is_rejected_before_database_mutation(engine):
    ...
    with Session(engine) as session:
        with session.begin():
            with pytest.raises(MigrationBypassError): execute_migration(session, definition)
    state, upgrades = _authority(engine)
    assert state.current_version == 1 and upgrades == []
...
def test_concurrent_same_identity_serializes_and_only_one_applies(engine):
    ...
    def worker():
        try:
            with Session(engine) as session:
                with session.begin():
                    barrier.wait(timeout=10); results.append(execute_migration(session, definition).status.value)
