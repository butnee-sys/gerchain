...
def test_shuud_reaches_exim_only_after_service_validation():
    ...
    flow = SHUUDEximFlow(exim=EXIMGateway(Downstream()), service_center=center)
    response = flow.handle(make_request(
        incident_id="I-1", vehicle_id="V-1", compensation=1_000_000, evidence_verified=True
    ))
...
def test_invalid_shuud_request_never_reaches_exim():
    ...
    flow = SHUUDEximFlow(exim=EXIMGateway(Downstream()), service_center=center)
    response = flow.handle(make_request(
        incident_id="I-2", vehicle_id="V-2", compensation=1_000_000, evidence_verified=False
    ))
