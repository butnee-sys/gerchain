...
def test_measurement_summary_api_exposes_timing_and_economics() -> None:
    ...
    from shuud.api import _INCIDENTS, _WITNESSES, _EVIDENCE, _DECISIONS

    # Keep the test on the public API for creation/measurement while using the
    # existing persistence adapter to inject a deterministic settlement milestone.
    snapshot = _PERSISTENCE.load_snapshot(incident_id)
    assert snapshot is not None
    timing = snapshot["operational_timing"]
    ...
    response = client.get(f"/api/v1/shuud/metrics/{incident_id}/summary", params={
